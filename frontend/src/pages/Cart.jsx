...
const Cart = () => {
  ...
  const deletItem = async (bookid) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/user/remove-from-cart/${bookid}`, {}, { headers });
...
const PlaceOrder = async () => {
  try{
    const response = await axios.post("http://localhost:3000/api/user/place-order", 
      {order: Cart}, 
      { headers }
    );
    alert(response.data.message);
